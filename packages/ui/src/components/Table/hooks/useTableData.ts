...
function useTableProps<T extends TableItem>(
  ...
  TableProps<T>,
  'columnConfig' | 'rowConfig' | 'rowHeaderColumn' | 'selection' | 'emptyState'
> {
...
/** @public */
export function useTableData<T extends TableItem, TFilter = unknown>(
  options: UseTableDataOptions<T, TFilter>,
): UseTableDataResult<T, TFilter> {
