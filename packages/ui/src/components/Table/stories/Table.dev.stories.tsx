...
export const BasicLocalData: Story = {
  render: () => {
    const columns: ColumnConfig<(typeof data1)[0]>[] = [
      {
        id: 'name',
        label: 'Name',
        cell: item => (
    ...
    return (
      <Table columnConfig={columns} rowHeaderColumn="name" {...tableProps} />
    );
  },
};
...
export const Search: Story = {
  render: () => {
    ...
    const { tableProps, search } = useTableData({
      mode: 'complete',
      getData: () => data1,
      paginationOptions: { pageSize: 5 },
...
export const RowLinks: Story = {
  render: () => {
    const columns: ColumnConfig<(typeof data4)[0]>[] = [
      {
        id: 'name',
        label: 'Band name',
...
export const Reload: Story = {
  render: () => {
    ...
    const { tableProps, reload } = useTableData({
      mode: 'complete',
      getData: () => data1,
      paginationOptions: { pageSize: 5 },
    });
...
export const ComprehensiveServerSide: Story = {
  render: () => {
    ...
    const { tableProps, search, filter } = useTableData<
      (typeof data1)[0],
      TypeFilter
    >({
      mode: 'offset',
